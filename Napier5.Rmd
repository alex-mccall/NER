...
google_checked <- ask_google(interesting_data[['locations']],dataset = dataset)
...
new_parsed_text <- prep_new_parsed_text(new_geographies,
                                        interesting_data[['parsedtxt']],
                                        dataset)
e <- new_parsed_text[1:50,]
data_set <- sentence_representation(e,dataset)
data_set <- sentence_representation(new_parsed_text,dataset)
split_data(data_set,dataset)
...
X_train,Y_train,X_dev, Y_dev = prep_python_env()
...
placenames <- unique(filter(ner_results,str_starts(predictions,"GPE"),
                            comp == TRUE))
pic <- draw_confusion_matrix(confusion_matrix)
