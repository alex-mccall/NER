...
sequences of labels (the hidden state Y) and
attributes of the observations corresponding to the labels (X).
...
```{r}
library(data.table)
prep_lines <- function(x) {
x <- as.data.table(x)
x <- x[, prevTag   := shift(pos, n = 1, type = "lag"), by = list(doc_id)]
x <- x[, prevSecondTag   := shift(pos, n = 2, type = "lag"), by = list(doc_id)]
x <- x[, nextTag   := shift(pos, n = 1, type = "lead"), by = list(doc_id)]
x <- x[, nextNextTag   := shift(pos, n = 2, type = "lead"), by = list(doc_id)]
x <- x[, prevWord  := shift(token, n = 1, type = "lag"), by = list(doc_id)]
x <- x[, prevSecondWord  := shift(token, n = 2, type = "lag"), by = list(doc_id)]
x <- x[, nextWord  := shift(token, n = 1, type = "lead"), by = list(doc_id)]
x <- x[, nextNextWord  := shift(token, n = 2, type = "lead"), by = list(doc_id)]
x <- x[, prevFound  := shift(found, n = 1, type = "lag"), by = list(doc_id)]
x <- x[, nextFound  := shift(found, n = 1, type = "lead"), by = list(doc_id)]
x <- x[, prevCapital  := shift(Capital, n = 1, type = "lag"), by = list(doc_id)]
x <- x[, prevSecondCapital  := shift(Capital, n = 2, type = "lag"), by = list(doc_id)]
x <- x[, nextCapital  := shift(Capital, n = 1, type = "lead"), by = list(doc_id)]
x <- x[, nextNextCapital  := shift(Capital, n = 2, type = "lead"), by = list(doc_id)]


x <- x[, prevTag   := txt_sprintf("tag[w-1]=%s", prevTag), by = list(doc_id)]
x <- x[, prevSecondTag   := txt_sprintf("tag[w-2]=%s", prevSecondTag), by = list(doc_id)]
x <- x[, nextTag   := txt_sprintf("tag[w+1]=%s", nextTag), by = list(doc_id)]
x <- x[, nextNextTag   := txt_sprintf("tag[w+2]=%s", nextNextTag), by = list(doc_id)]
x <- x[, prevWord  := txt_sprintf("word[w-1]=%s", prevWord), by = list(doc_id)]
x <- x[, prevSecondWord  := txt_sprintf("word[w-2]=%s", prevSecondWord), by = list(doc_id)]
x <- x[, nextWord  := txt_sprintf("word[w+1]=%s", nextWord), by = list(doc_id)]
x <- x[, nextNextWord  := txt_sprintf("word[w+2]=%s", nextNextWord), by = list(doc_id)]
x <- x[, prevFound := txt_sprintf("found[w-1]=%s", prevFound), by = list(doc_id)]
x <- x[, nextFound := txt_sprintf("found[w+1]=%s", nextFound), by = list(doc_id)]
x <- x[, prevCapital := txt_sprintf("cap[w-1]=%s", prevCapital), by = list(doc_id)]
x <- x[, prevSecondCapital  := txt_sprintf("cap[w-2]=%s", prevSecondCapital), by = list(doc_id)]
x <- x[, nextCapital  := txt_sprintf("cap[w+1]=%s", nextCapital), by = list(doc_id)]
x <- x[, nextNextCapital  := txt_sprintf("cap[w+2]=%s", nextNextCapital), by = list(doc_id)]
...
npt <- mutate(new_parsed_text, Capital = str_detect(token, "^[:upper:].+$")
               #Placefeats =  TRUE %in% str_detect(token,place_feat)
               )
npt1 <- prep_lines(npt)
subset(npt1, doc_id == 100, select = c("doc_id", "token", "token_previous", "token_next"))
