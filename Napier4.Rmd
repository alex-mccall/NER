...
locations <- select(locations,doc_id,sentence_id,token_id,entity,result) %>%
  filter(entity == "GPE_B")
...
geographies_complete <- filter(proper_noun,entity !="") %>%
  mutate(entity = ifelse(str_detect(entity,regex("[A-Z]+_B")),"GPE_B",
                          ifelse(str_detect(entity,regex("[A-Z]+_I")),"GPE_I","O")))
locations <- get_locations(geographies_complete)
interesting_data <- list(geographies_complete,organisations,persons,noun,proper_noun,locations,text,parsedtxt)
...
#aveRDS(new_geographies,filename)
...
#} else {
df <-  select(df,doc_id,sentence_id,token,tag,entity,tagentity,entityo) %>%
  mutate(sent_id = as.numeric(paste0(str_extract(doc_id,'[0-9]+'),
                                     as.character(sentence_id)))) %>%
  select(sent_id,token,tag,entity,tagentity,entityo)
mylist <- list()
...
for(line in unique(df$sent_id)) {
  ...
  entityo <- a$entityo
  newline <- list(sentence,tagentity,entityo)
...
len <- length(data_set)
if(train_model) {
data_split <- as.integer(length(data_set)*1/4)
test_data <- data_set[1:data_split]
train_data <- data_set[data_split:len]
...
get_feats <- function(dta) {
  ...
  for(sent in dta) {
    if(length(sent[[1]]) > 2) {
      ...
      labels <- append(labels,list(sent[[3]]))
    } else {
