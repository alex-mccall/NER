...
```{r}
split_data <- function(data_set,dataset) {
...
get_result_data <- function(dataset) {
  ...
  saved_data <- list(X_train,Y_train,X_dev,Y_dev,y_pred)
  names(saved_data) <- c('X_train','Y_train','X_dev','Y_dev','y_pred')
  return(saved_data)
}
```
...
def prep_python_env():
  ...
  exists = os.path.isfile(X_train_file)
  if exists :
